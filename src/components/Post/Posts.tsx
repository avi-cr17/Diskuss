...
const Posts: React.FC<PostsProps> = ({
  ...
  const search = useRecoilValue(searchInputState);
  ...
  useEffect(() => {
    ...
    // return () => unsubscribe();
  }, [communityData, postStateValue.postUpdateRequired, search]);
  ...
  const getPosts = async () => {
    ...
    try {
      ...
      console.log("communtiy", posts)

      if(search){
      filteredArray = await posts.filter((post)=>{
       return post.title.toLowerCase().includes(search.query.toLocaleLowerCase()); 
      });
      ...
      setPostStateValue((prev) => ({
        ...
        postsCache: {
          ...prev.postsCache,
          [communityData?.id!]: filteredArray as Post[],
        },
