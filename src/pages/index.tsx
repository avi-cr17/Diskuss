...
const Home: NextPage = () => {
  ...
  const queryString = useRecoilValue(searchInputState);
  let search = queryString.query;
  ...
  const getUserHomePosts = async () => {
    ...
    try {
      ...
      if (communityStateValue.mySnippets.length) {
        ...
        const communityLength = myCommunityIds.length;
        console.log(myCommunityIds);
        for (var id in myCommunityIds){
          if (!id) return;
          //console.log(id)
          postPromises.push(
            getDocs(
              query(
                ...
                where("communityId", "==", myCommunityIds[id]),
                limit(10)
              )
            ) 
          );
        ...
        queryResults.forEach((result) => {
         ...
         feedPosts.push(...filteredArray);
         console.log("user feed",filteredArray)
         
        });

        // let filteredArray = posts;
        // //console.log("no username", posts)
  
        // if(search){
        // filteredArray = await posts.filter((post)=>{
        //  return post.title.toLowerCase().includes(search.toLowerCase()); 
        // });
  
        // }
        // console.log("filtered no user" ,filteredArray)
      ...
      console.log("HERE ARE FEED POSTS", );
  ...
  }, [user, communityStateValue.initSnippetsFetched, search]);
